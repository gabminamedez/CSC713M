...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for idx1, i in enumerate(X):
          for idx2, j in enumerate(self.X_train):
            dists[idx1, idx2] = np.sqrt(np.sum(np.square(i - j)))
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            #########################################################################
            argsort_idxs = np.argsort(dists[i])
            for j in range(k):
              closest_y.append(self.y_train[argsort_idxs[j]])
    ...
    def compute_distances_one_loop(self, X):
        ...
        for idx, i in enumerate(X):
          dists[idx, :] = np.sqrt(np.sum(np.square(i - self.X_train), axis=1))
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
