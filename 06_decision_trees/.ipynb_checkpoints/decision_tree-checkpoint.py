...
class DecisionTree(object):
    ...
    def create_tree(self, X, y, depth=0):
        ...
        labels = y.tolist()
        node = TreeNode()
        if labels.count(labels[0]) == len(labels) or depth == self.max_depth + 1 or len(y) < self.min_samples_split:
            node.value = self.compute_leaf(y)
            node.node_type = "Leaf"
            
            return node
        else:
            best_imp, best_idx, best_threshold, split_idx = self.choose_best_feature_split(X, y)
            
            if best_imp < self.min_impurity or len(split_idx["left_idx"]) == 0 or len(split_idx["right_idx"]) == 0:
                node.value = self.compute_leaf(y)
                node.node_type = "Leaf"
            ...
            node.left_branch = self.create_tree(X[split_idx["left_idx"]], y[split_idx["left_idx"]], depth + 1)
            node.right_branch = self.create_tree(X[split_idx["right_idx"]], y[split_idx["right_idx"]], depth + 1)
    ...
    def choose_best_feature_split(self, X, y):
        ...
        num_parent = [np.sum(y == c) for c in range(len(np.unique(y)))]
        ...
        for idx in range(D):
            ...
            for i in range(1, m):
                ...
                impurity = self.compute_impurity(num_parent, num_left, num_right)
                ...
                if impurity < max_impurity:
                    ...
                    split_idx["left_idx"] = num_left
                    split_idx["right_idx"] = num_right
        #########################################################################
        #                              END OF YOUR CODE                         #
        #########################################################################
