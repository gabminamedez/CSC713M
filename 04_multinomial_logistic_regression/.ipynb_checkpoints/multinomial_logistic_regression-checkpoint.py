...
class MultinomialLogisticRegression(object):
    ...
    def predict(self, X):
        ...
        #############################################################################
        probs = self.softmax(np.dot(X, self.params['W']) + self.params['b'])
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        score = self.softmax(np.dot(X, W) + b)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        ohl = np.zeros((len(y), np.max(y) + 1))
        ohl[np.arange(len(y)), y] = 1
        loss = (-1 / N) * np.sum(ohl * np.log(score)) + (lambda_reg / 2) * D
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        grads['W'] = (-1 / N) * np.dot(X.T, (ohl - score)) + lambda_reg * D
        grads['b'] = -np.sum((ohl - score), axis=0) / N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
